...
function Home(props) {
  ...
  return (
    <>
      <div className="pb-10">
        <CurrentBanner />
        {NFTs.length> 0 && <div className="px-14">
          <Carousel items={NFTs} className="" onCardClick={NFTClickHandler} />
        </div>}
        <TrendingNow items={recommendations} />
        {live.length>0 &&<div className="py-3 px-4 text-center font-bold text-lg border-b-2 border-tertiaryred-50   text-tertiarywhite-50  mx-[4%] w-[10%]">
          Live Now
        </div>
         &&<div className="px-16">
      ...
      {NFT && Object.keys(NFT)?.length > 0 && (
        <GenericModal
          ...
          posText={NFT.forSale && NFT.currentOwner != blockChainCtx.accountAddress ? "Buy" : "Ok"}
          negText="Cancel"
